...
// en asignar un arreglo con las notas y uno de igual tamano con las duraciones, para luego reproducir
// uno por una.
//
...
void setup() {
  ...
  for (int thisNote = 0; thisNote < 112; thisNote++) {
    ...
    Zeego.Buzzer.playTone(melody[thisNote], noteDuration);

    // para distinguir cada nota, se calcula un tiempo minimo entre ellas.
    // la duracion de la nota + 30% suena bien.
    int pauseBetweenNotes = noteDuration * 1.30;
    delay(pauseBetweenNotes);
    Zeego.Buzzer.noTone();    // Al final, detiene la reproduccion de sonido.
  }
}
